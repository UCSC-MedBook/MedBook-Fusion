#!/usr/bin/env /data/home/galaxy/VV/Rgetopt.py
...
main <- function(argv) {
  ...
  o <- Rgetopt(argv=argv, argspec=argspec, defaults=list(ac=1))
  ...
  # Features with low variance foil SAM, filter them out.
  v = apply(data,1,var)
  data = data[v >= 0.001,]
  data.df = data.df[v >= 0.001,]
